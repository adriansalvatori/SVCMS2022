...
/**
 * PH Logging Class
 * Based on project-huddle by Eric Binnion
 * https://github.com/ebinnion/WP-Logger
...
class PH_Logger
{
	...
	function __construct()
	{
		...
		if (isset(self::$instance)) {
			wp_die(esc_html__('The PH_Logger class has already been loaded.', 'project-huddle'));
		}
		...
		add_action('admin_menu',            array($this, 'add_menu_page'), 200);
		add_action('admin_enqueue_scripts', array($this, 'enqueue_scripts'));

		// These actions allow developers to add log entries, purge log entries for a plugin, and create/end sessions.
		add_action('ph_logger_add',            array($this, 'add_entry'), 10, 4);
		add_action('ph_logger_purge',          array($this, 'purge_plugin_logs'));
		add_action('ph_logger_create_session', array($this, 'create_set_session'), 10, 4);
		add_action('ph_logger_end_session',    array($this, 'end_session'));
		 ...
		 */
		add_filter('ph_logger_version', array($this, 'get_ph_logger_version'));
	...
	/**
	 * Will return the current version of WP Logger to plugins that call the `ph_logger_version` filter.
	 *
	 * @param  null
	 * @return string The current version of WP Logger
	 */
	function get_ph_logger_version($version)
	{
		return '0.1';
	}

	/**
	 * Callback function for 'ph_logger_purge' that will delete all log entries (comments) for a given plugin.
	 *
	...
	/**
	 * Register the project-huddle post type and plugin-messages taxonomy. Also, process bulk delete action and log mailing.
	 */
	function init()
	{
		register_post_type(
			...
			array(
				...
				'supports'      => false,
				'labels'        => array(
					'name'               => esc_html__('Logs', 'project-huddle'),
					'singular_name'      => esc_html__('Log', 'project-huddle'),
					'add_new'            => esc_html__('Add New Log', 'project-huddle'),
					'add_new_item'       => esc_html__('Add New Log', 'project-huddle'),
					'edit_item'          => esc_html__('Edit Log', 'project-huddle'),
					'new_item'           => esc_html__('Add New Log', 'project-huddle'),
					'view_item'          => esc_html__('View Log', 'project-huddle'),
					'search_items'       => esc_html__('Search Logs', 'project-huddle'),
					'not_found'          => esc_html__('No logs found', 'project-huddle'),
					'not_found_in_trash' => esc_html__('No logs found in trash', 'project-huddle')
				),
				'capabilities' => array(
		...
		if (is_admin() && isset($_GET['page'])) {
			...
			if ('ph_logger_messages' == $_GET['page'] && isset($_POST['action']) && 'delete' == $_POST['action']) {

				check_admin_referer('ph_logger_generate_report', 'ph_logger_form_nonce');
	...
	{
		if ('ph-project_page_ph_logger_messages' == $hook) {
			wp_enqueue_script('ph_logger', plugins_url('js/script.js', __FILE__), array('jquery'));
			wp_enqueue_style('ph_logger', plugins_url('css/style.css', __FILE__));
		}
	}
	...
	function generate_menu_page()
	{
		...
		require_once(trailingslashit(dirname(__FILE__)) . 'lib/class-ph-logger-list-table.php');
		...
		$hide_form     = isset($_COOKIE['ph_logger_hide_form']) ? 'hide-form' : '';

		$logger_table  = new PH_Logger_List_Table($this->get_entries());
	...
	public function process_email_log()
	{
		...
		check_ajax_referer('ph_logger_generate_report', 'ph_logger_form_nonce');
		...
		if (!empty($entries)) {
				...
				if (!empty($entry->session)) {
					...
					$sessions = $comments_query->query(
						array(
							...
							'comment_approved' => 'project-huddle'
						)
					);
			...
			$filename = 'project-huddle-' . sanitize_title($current_site) . '-' . $time;

			// Make sure that wp-content/project-huddle exists, if not, create it.
			if (!is_dir(WP_CONTENT_DIR . '/project-huddle')) {
				mkdir(WP_CONTENT_DIR . '/project-huddle');
			}

			// Test again to make sure that wp-content/project-huddle exists before attempting to open a file in the directory.
			if (is_dir(WP_CONTENT_DIR . '/project-huddle')) {
				$file = fopen(WP_CONTENT_DIR . "/project-huddle/{$filename}.json", 'w');
				...
				if (false !== $file) {
					...
					$send_email = wp_mail(
						$plugin_email,
						sprintf(__('Logs from %s', 'project-huddle'), $current_site),
						sprintf(__('Attached is a log in JSON format from %s', 'project-huddle'), $current_site),
						'',
						array(WP_CONTENT_DIR . "/project-huddle/{$time}.json")
					);
				} else {
					$send_email = wp_mail(
						$plugin_email,
		...
		if (isset($send_email) && $send_email) : ?>
			...
			<div class="updated">
				<p><?php esc_html_e('Your message was sent successfully!', 'project-huddle'); ?></p>
			</div>

		<?php else : ?>

			<div class="error">
				<p><?php esc_html_e('Your message failed to send.', 'project-huddle'); ?></p>
			</div>
	...
	private function build_log_select($plugin_name, $log_id = false)
	{
		...
		?>
			...
			<select id="log-select" name="log-select">
				<option value=""><?php esc_html_e('All Logs', 'project-huddle'); ?></option>
...
<?php
	...
	private function limit_plugin_logs($plugin_name, $log_name, $log_id)
	{
		 ...
		 * @param string $log_name The log for the current entry.
		 */
		$limit = apply_filters('ph_logger_limit_' . $plugin_name, 150, $log_name);

		$comments = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT * FROM $wpdb->comments WHERE comment_approved = 'project-huddle' AND comment_author = %s AND comment_post_ID = %d ORDER BY comment_date ASC",
				$plugin_name,
				$log_id
			)
		);
	...
	/**
	 * Prefixes a string with 'project-huddle-' or $plugin_name
	 *
	 * @param  string $slug Plugin slug.
	 * @param  string $plugin_name If set, slug will be prefixed with plugin_name
	 *
	 * @return string String that being with 'project-huddle-'
	 */
	...
	private function get_plugin_email($plugin_name)
	{
		 ...
		 */
		return 'support@projecthuddle.com'; //apply_filters( "ph_logger_author_email_{$plugin_name}", '' );
	}
	...
	private function get_entries($limit = 20)
	{
		...
		$post_where = "post_type = 'project-huddle' AND post_parent != 0";
		$comment_where = "comment_approved = 'project-huddle'";
