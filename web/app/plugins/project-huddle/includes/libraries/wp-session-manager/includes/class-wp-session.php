...
final class WP_Session extends Recursive_ArrayAccess
{
	...
	{
		$secure = apply_filters('wp_session_cookie_secure', true);
		$httponly = apply_filters('wp_session_cookie_httponly', false);
		$value = $this->session_id . '||' . $this->expires . '||' . $this->exp_variant;
		$path = COOKIEPATH;

		if (PHP_VERSION_ID < 70300) {
			setcookie(WP_SESSION_COOKIE, $value, $this->expires, "$path; samesite=None", COOKIE_DOMAIN, $secure, $httponly);
		} else {
			setcookie(WP_SESSION_COOKIE, $value, [
