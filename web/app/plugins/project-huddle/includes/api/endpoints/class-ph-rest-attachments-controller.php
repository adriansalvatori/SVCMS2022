...
/**
 * REST API: PH_REST_Attachments_Controller class
 *
 * @package WordPress
 * @subpackage REST_API
 * @since 4.7.0
 ...
 * @since 4.7.0
 *
 * @see PH_REST_Posts_Controller
 */
class PH_REST_Attachments_Controller extends PH_REST_Posts_Controller
{
	/**
	 * Check if filename should be blacklisted
	 */
	protected function is_blacklisted($filename)
	{
		$blacklist = ph_blacklist_filetypes();
		//extract and check extensions from filename
		$parts = explode(".", $filename);
		$exists = false;
		for ($i = count($parts) - 1; $i > max(0, count($parts) - 3); $i--) {
			$exists = isset($blacklist[$parts[$i]]);
			if ($exists) break;
		}
		return $exists;
	}
	...
	protected function check_delete_permission($post)
	{
		...
		// let user delete their own attachments
...
function ph_register_attachment_routes()
{
	$controller = new PH_REST_Attachments_Controller('attachment');
	$controller->register_routes();
}

add_action('rest_api_init', 'ph_register_attachment_routes');
