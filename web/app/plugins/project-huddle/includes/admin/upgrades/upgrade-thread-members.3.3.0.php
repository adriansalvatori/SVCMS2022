...
final class PH_Upgrade_Thread_Members extends PH_Upgrade {
	/**
	 * Unique Identifier for upgrade routine
	 * @var string
	 */
	public $name = 'data-threads-members';
	 ...
	 * @var string
	 */
	public $nice_name = '3.3.0 Email Notification Data Upgrade';
	 ...
	 * @var string
	 */
	public $priority = '3.3.0';

	/**
	 * Upgrade user-facing description
	 * @var string
	 */
	public $description = 'An update is necessary for comment thread data changes to version 3.3.0. This may take a while if you have a lot of threads. Please don\'t navigate away from this page.';
	...
	public function complete() {
		ph_log( '3.3.0 Thread Members Data Update Completed' );
	...
	public function isComplete() {
		...
		if ( version_compare( get_site_option( 'ph_data_upgrade_version' ), '3.2.99', '<' ) ) {
			return false;
		}
...
function ph_register_upgrade_thread_members( $upgrades ) {
	$upgrades[] = new PH_Upgrade_Thread_Members();

	return $upgrades;
}

add_action( 'ph_upgrade_handler_register', 'ph_register_upgrade_thread_members' );
