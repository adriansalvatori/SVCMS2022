...
class ACF_USER_POST_FIELD_UPDATED {
	...
	public function __construct() {
		...
		$this->trigger_code = 'ACF_USER_POST_FIELD_UPDATED';
		$this->trigger_meta = 'ACF_USER_POST_FIELD_UPDATED_META';
	...
	public function define_trigger() {
		...
		$trigger = array(
			...
			/* translators: ACF Field trigger */
				__( 'A user updates {{a field:%1$s}} on {{a post:%2$s}}', 'uncanny-automator-pro' ),
				'ACF_FIELD_LIST:' . $this->trigger_meta,
				'ACF_WP_POSTS_OBJECT_LIST:' . $this->trigger_meta
			),
			/* translators: ACF Field trigger */
			'select_option_name'  => __( 'A user updates {{a field}} on {{a post}}', 'uncanny-automator-pro' ),
