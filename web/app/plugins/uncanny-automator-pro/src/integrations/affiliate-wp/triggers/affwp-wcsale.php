...
class AFFWP_WCSALE {
	...
	public function define_trigger() {
		...
		$trigger = array(
			...
			'sentence'            => sprintf( __( 'An affiliate refers a sale of {{a WooCommerce product:%1$s}}', 'uncanny-automator-pro' ), $this->trigger_meta ),
			/* translators: Logged-in trigger - Affiliate WP */
			'select_option_name'  => __( 'An affiliate refers a sale of {{a WooCommerce product}}', 'uncanny-automator-pro' ),
			'action'              => 'affwp_insert_referral',
			'priority'            => 99,
			'accepted_args'       => 1,
			'validation_function' => array( $this, 'refers_wc_sale' ),
			'options'             => [
				$uncanny_automator->helpers->recipe->affiliate_wp->options->pro->get_wc_products( null, $this->trigger_meta, [ 'any_option' => true ] ),
			],
		);
