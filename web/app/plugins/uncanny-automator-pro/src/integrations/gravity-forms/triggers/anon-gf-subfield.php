	...
	public function gform_submit( $entry, $form ) {
		...
		if ( ! empty( $conditions ) ) {
			foreach ( $conditions['recipe_ids'] as $trigger_id => $recipe_id ) {
				if ( ! Automator()->process->anon->is_anon_recipe_completed( $recipe_id, $user_id, true ) ) {
					$pass_args = array(
						'code'             => $this->trigger_code,
					...
					if ( isset( Automator()->process ) && isset( Automator()->process->anon ) && Automator()->process->anon instanceof Automator_Pro_Recipe_Process_Anon ) {
						$args = Automator()->process->anon->maybe_add_anon_trigger_entry( $pass_args, false );
					} else {
