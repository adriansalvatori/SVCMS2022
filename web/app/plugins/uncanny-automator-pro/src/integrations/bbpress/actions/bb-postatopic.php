...
class BB_POSTATOPIC {
	...
	public function define_action() {
		...
		$action = [
			...
			'options_group'      => [
				$this->action_meta => [
					$uncanny_automator->helpers->recipe->bbpress->options->list_bbpress_forums( esc_attr__( 'Forum', 'uncanny-automator-pro' ), $this->action_meta, false ),
					...
					$uncanny_automator->helpers->recipe->field->text_field( 'BBTOPICCONTENT', esc_attr__( 'Topic content', 'uncanny-automator-pro' ), true, 'textarea' ),
				],
			],
		];
	...
	public function post_a_forum_topic( $user_id, $action_data, $recipe_id, $args ) {
		...
		$topic_title    = $uncanny_automator->parse->text( $action_data['meta']['BBTOPICTITLE'], $recipe_id, $user_id, $args );
		$topic_title    = do_shortcode( $topic_title );
		$topic_content  = $uncanny_automator->parse->text( $action_data['meta']['BBTOPICCONTENT'], $recipe_id, $user_id, $args );
