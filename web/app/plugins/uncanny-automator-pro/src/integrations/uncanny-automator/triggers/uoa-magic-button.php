...
class UOA_MAGIC_BUTTON {
	...
	public function define_trigger() {
		...
		$trigger = array(
			...
			'support_link'        => $uncanny_automator->get_author_support_link( $this->trigger_code, 'knowledge-base/magic-button/' ),
			'is_pro'              => true,
			...
			foreach ( $results as $result ) {
				if ( true === $result['result'] && $result['args']['trigger_id'] && $result['args']['get_trigger_id'] ) {
					...
					if ( ! empty( $_POST ) && isset( $_POST['automator_button_post_id'] ) ) {
						...
						$save_meta['meta_value'] = $uncanny_automator->uap_sanitize( $_POST['automator_button_post_id'] );
						$uncanny_automator->insert_trigger_meta( $save_meta );
					}

					if ( ! empty( $_POST ) && isset( $_POST['automator_button_post_title'] ) ) {
						$save_meta['meta_key']   = 'automator_button_post_title';
						$save_meta['meta_value'] = $uncanny_automator->uap_sanitize( $_POST['automator_button_post_title'] );
						$uncanny_automator->insert_trigger_meta( $save_meta );
					}
