...
/**
 * Class BDB_INVITEMARKACTIVATEUSER
...
class BDB_INVITEMARKACTIVATEUSER {
	...
	public function __construct() {
		$this->trigger_code = 'BDBINVITEMARKACTIVATEUSER';
	...
	public function define_trigger() {
		...
		$trigger = array(
			...
			'sentence'            => esc_attr__( "A user's email invitation results in a new member activation", 'uncanny-automator-pro' ),
			/* translators: Logged-in trigger - BuddyBoss */
			'select_option_name'  => esc_attr__( "A user's email invitation results in a new member activation", 'uncanny-automator-pro' ),
		...
		if ( $args ) {
			foreach ( $args as $result ) {
				if ( true === $result['result'] && $result['args']['trigger_id'] && $result['args']['get_trigger_id'] ) {
					...
					$run_number = $uncanny_automator->get->trigger_run_number( $result['args']['trigger_id'], $result['args']['get_trigger_id'], $inviter_id );
					$save_meta  = [
						'user_id'        => $inviter_id,
