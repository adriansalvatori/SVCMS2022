...
class BDB_POSTATOPICREPLY {
	...
	public function define_action() {
		...
		$action = [
			...
			'sentence'           => sprintf( esc_attr__( 'Post a reply to {{a topic:%1$s}} in {{a forum:%2$s}}', 'uncanny-automator-pro' ), 'BDBTOPIC', $this->action_meta ),
			/* translators: Action - BuddyBoss */
			...
			'options_group'      => [
				$this->action_meta => [
					...
					$uncanny_automator->helpers->recipe->field->select_field( 'BDBTOPIC', __( 'Topic', 'uncanny-automator-pro' ), [], false, false, false ),
					$uncanny_automator->helpers->recipe->field->text_field( 'BDBREPLYCONTENT', esc_attr__( 'Reply content', 'uncanny-automator-pro' ), true, 'textarea' ),
				],
			],
		];
	...
	public function post_a_forum_topic_reply( $user_id, $action_data, $recipe_id, $args ) {
		...
		$topic_id       = $uncanny_automator->parse->text( $action_data['meta']['BDBTOPIC'], $recipe_id, $user_id, $args );
		$topic_id       = do_shortcode( $topic_id );
		$reply_content  = $uncanny_automator->parse->text( $action_data['meta']['BDBREPLYCONTENT'], $recipe_id, $user_id, $args );
