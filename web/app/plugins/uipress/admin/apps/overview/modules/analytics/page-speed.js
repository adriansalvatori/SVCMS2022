    ...
    methods: {
      createChartData() {
        ...
        chartdataset = {
          ...
          datasets: [
            {
              label: self.overviewData.translations.pageSpeed,
              fill: true,
              data: pageviewsdata.timeline.pageLoadTime,
              backgroundColor: ["rgba(12, 92, 239, 0.05)"],
              borderColor: ["rgba(12, 92, 239, 1)"],
              borderWidth: 2,
              chartTitle: self.overviewData.translations.pageSpeed,
            ...
            },
            {
              label: self.overviewData.translations.pageSpeed,
              fill: true,
              data: pageviewsdatacomp.timeline.pageLoadTime,
      ...
      returnToFixed(item, dec) {
        return Number(item).toFixed(dec);
      },
    },
    template:
        ...
        <template v-else>\
          ...
          <div v-if="!overviewData.ui.editingMode && overviewData.globalDataObject.loading != true && isGAconnected">\
            <div class="uip-flex uip-flex-center uip-margin-bottom-xs">\
              <div class="uip-margin-right-s uip-text-xxl uip-text-emphasis uip-text-bold">{{returnToFixed(tableData.report.totals.pageLoadTime, 2)}}s</div>\
              ...
              :class="{\'uip-background-red-wash uip-text-danger\' : tableData.report.totals_change.pageLoadTime < 0}">\
                <span v-if="tableData.report.totals_change.pageLoadTime > 0" class="material-icons-outlined" >expand_less</span>\
                <span v-if="tableData.report.totals_change.pageLoadTime < 0" class="material-icons-outlined" >expand_more</span>\
              </div>\
              <div class="uip-text-bold uip-text-green" \
              :class="{\'uip-text-danger\' : tableData.report.totals_change.pageLoadTime < 0}">{{tableData.report.totals_change.pageLoadTime}}%</div>\
            </div>\
            <div class="uip-margin-bottom-m">\
                <div class="uip-text-muted">{{overviewData.translations.comparedTo}}: {{overviewData.dateRange.startDate_comparison}} - {{overviewData.dateRange.endDate_comparison}} ({{returnToFixed(tableData.report_comparison.totals.pageLoadTime, 2)}}s)</div>\
            </div>\
