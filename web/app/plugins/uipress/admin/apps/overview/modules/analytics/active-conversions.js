    ...
    methods: {
      createChartData() {
        ...
        for (var i = 0; i < thisreport.data.length; i++) {
          ...
          chartdataset[key] = item["conversions"];
        }
        ...
        chartdataset = {
          ...
          datasets: [
            {
              label: self.overviewData.translations.conversions,
              fill: true,
              ...
              borderRadius: 10,
              maxBarThickness: 20,
              chartTitle: self.overviewData.translations.conversions,
        ...
        <template v-else>\
          ...
          <div v-if="!overviewData.ui.editingMode && overviewData.globalDataObject.loading != true && isGAconnected" >\
            <div class="uip-flex uip-flex-center uip-margin-bottom-m">\
              <div class="uip-margin-right-s uip-text-xxl uip-text-emphasis uip-text-bold">{{tableData.report.totals.conversions}}</div>\
            </div>\
