...
class HelperService
{
     ...
     * @throws Exception
     */
    public function getProviderCabinetUrl($email, $type, $dateStartString, $dateEndString)
    {
        /** @var SettingsService $cabinetSettings */
    ...
    public function getBookingTranslation($bookingInfo, $entityTranslation, $type)
    {
        ...
        if ($bookingInfo && !empty($bookingInfo['locale'])) {
            $locale = $this->getLocaleLanguage($usedLanguages, $bookingInfo['locale']);
    ...
    {
        $similarLanguageIndex = array_search(
            current(explode('_', $locale)),
            array_map(
                function ($val) {
                    return current(explode('_', $val));
                },
                $usedLanguages
            )
        );

        return $similarLanguageIndex === false ? $locale : $usedLanguages[$similarLanguageIndex];
    }
